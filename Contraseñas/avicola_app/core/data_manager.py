    ...
    def _validate_structure(self, data):
        ...
        for module_name, lots in data.items():
            ...
            for lote_name, lote_data in lots.items():
                ...
                if "casetas" in lote_data and not isinstance(
                    lote_data["casetas"], dict
                ):
                    return False

                if "produccion" in lote_data and not isinstance(
                    lote_data["produccion"], dict
                ):
                    return False
    ...
    def migrate_old_data(self, data):
        ...
        for module_name, lots in data.items():
            ...
            for lote_name, lote_data in lots.items():
                ...
                if (
                    isinstance(lote_data, dict)
                    and "casetas" in lote_data
                    and isinstance(lote_data["casetas"], dict)
                ):
                    ...
                    for caseta_name, corrales in lote_data["casetas"].items():
                        if isinstance(corrales, list):
                            casetas_corregidas[caseta_name] = [
                                {
                                    "nombre": str(
                                        corral.get("nombre", f"Corral {i+1}")
                                    ),
                    ...
                    migrated_data[module_name][lote_name] = {
                        "_info": lote_data.get(
                            "_info", {"fecha_alojamiento": "Sin fecha"}
                        ),
    ...
    def save_data(self):
        ...
        try:
            with open(self.data_file, "w", encoding="utf-8") as f:
                json.dump(
                    self.clean_data(self.modules.copy()),
                    f,
                    indent=4,
                    ensure_ascii=False,
                )
